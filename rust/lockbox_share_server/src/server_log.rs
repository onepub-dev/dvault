...
fn fallback_log_path() -> PathBuf {
    platform_log_dir().join("server.log")
}

#[cfg(windows)]
fn platform_log_destination() -> &'static str {
    "Windows Event Log source Lockbox Share Server"
}
...
fn write_platform_log(message: &str) -> std::io::Result<()> {
    ...
    INIT.call_once(|| {
        ...
        // process lifetime, as required by `openlog`.
        unsafe {
            libc::openlog(
                IDENT.as_ptr().cast(),
...
fn write_platform_log(message: &str) -> std::io::Result<()> {
    ...
    let source = to_wide("Lockbox Share Server");
