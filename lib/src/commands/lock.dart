...
class LockCommand extends Command<void> {
  @override
  String get description => '''Locks the passed in file by adding it to a vault.
  ...
  LockCommand() {
    argParser.addOption('vault',
        abbr: 'v',
        help: '''The path and filename of the vault to store the file into.
  ...
  void run() {
    ...
    var incudeChildren = argResults!['children'] as bool;
    ...
    var filePaths = argResults!.rest;

    if (vaultPath == null) {
      if (filePaths.length > 1) {
        printerr(red(
            'As you have passed multiple paths you must pass a vault name via the -v option'));
        print(argParser.usage);
        exit(1);
      }
      var file = filePaths.first;
    ...
    if (exists(vaultPath)) {
      if (overwrite) {
        ...
        printerr(red(
            'The passed vault path ${truepath(vaultPath)} already exists.'));
        print(argParser.usage);
        exit(1);
      }
    }

    addToVault(filePaths, incudeChildren, vaultPath);
  }

  void addToVault(
      List<String> filePaths, bool incudeChildren, String vaultPath) {
    ...
    for (final filePath in filePaths) {
      ...
      if (isFile(filePath)) {
        ...
        vault.addDirectory(filePath, recursive: incudeChildren);
      } else {
